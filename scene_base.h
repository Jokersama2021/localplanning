...
public:
	...
	virtual void showScene();//显示，虚
	virtual void obsMoveStep() {}//虚，障碍物单帧移动
	virtual bool planning_process() = 0;//整个过程，纯虚
	...
	void driftTurnByRot(const double& total_theta, const Point& center);//漂移过弯（自转+公转），只考虑自转转角 
	void driftTurnByRev(const double& total_theta, const Point& center);//漂移过弯（自转+公转），只考虑公转转角 
